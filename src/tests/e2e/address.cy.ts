...
describe("Step 6 – Address Information", () => {
  beforeEach(goToAddress);

  it("shows the address form", () => {
  ...
  it("auto-fills city and state for known postal code 400001", () => {
    cy.get('input[placeholder="Enter postal code"]').type("400001");
    cy.get('input[placeholder="Auto-filled city"]').should("have.value", "Mumbai");
    cy.get('input[placeholder="Auto-filled state"]').should("have.value", "Maharashtra");
  });

  it("auto-fills for postal code 110001 (New Delhi)", () => {
    cy.get('input[placeholder="Enter postal code"]').type("110001");
    cy.get('input[placeholder="Auto-filled city"]').should("have.value", "New Delhi");
    cy.get('input[placeholder="Auto-filled state"]').should("have.value", "Delhi");
  });

  it("permanent address checkbox copies current address", () => {
    cy.get('input[type="checkbox"]').check();
    cy.get('input[type="checkbox"]').should("be.checked");
  });

  it("advances to Document Upload with valid data", () => {
